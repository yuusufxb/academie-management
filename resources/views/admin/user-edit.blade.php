    ...
    <form method="POST" action="{{ route('admin.users.update', $user['id'] ?? 1) }}">
        ...
        <div class="bg-white rounded-md shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div class="p-6 space-y-5">
                ...
                {{-- Name Input --}}
                <div>
                    <label class="block font-headline text-slate-500 text-xs font-bold uppercase mb-2 text-right">الاسم</label>
                    <input name="name" type="text" value="{{ old('name', $user['name'] ?? 'Stagaire') }}" class="form-ctrl font-headline" required/>
                </div>

                {{-- Email Input --}}
                <div>
                    <label class="block font-headline text-slate-500 text-xs font-bold uppercase mb-2 text-right">البريد الإلكتروني</label>
                    <input name="email" type="email" value="{{ old('email', $user['email'] ?? 'stage@aref.sm') }}" class="form-ctrl font-headline" style="text-align: left; direction: ltr;" required/>
                </div>

                {{-- Level Input --}}
                <div>
                    <label class="block font-headline text-slate-500 text-xs font-bold uppercase mb-2 text-right">المستوى</label>
                    <input name="level" type="number" value="{{ old('level', $user['level'] ?? 1) }}" min="1" max="3" class="form-ctrl font-headline" required/>
                </div>

                {{-- Institution Code Input --}}
                <div>
                    <label class="block font-headline text-slate-500 text-xs font-bold uppercase mb-2 text-right">رمز المؤسسة</label>
                    <input name="institution_code" type="text" value="{{ old('institution_code', $user['institution_code'] ?? '1666') }}" class="form-ctrl font-headline"/>
                </div>

                {{-- Footer Actions - Matching the structure of the edit screenshot --}}
                <div class="flex items-center justify-end gap-3 pt-5 mt-5 border-t border-slate-50">
                    ...
                    {{-- Update Button (Theme Blue/Green Style) --}}
                    <button type="submit" 
